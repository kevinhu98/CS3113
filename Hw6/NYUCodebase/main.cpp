...
int main(int argc, char *argv[])
{
	...
	Mix_Chunk *pongBounce;
	Mix_Chunk *pongLose;
	Mix_Music *music;

	pongBounce = Mix_LoadWAV("pongBounce.wav");
	pongLose = Mix_LoadWAV("pongLose.wav");
	...
	while (!done) {
		...
		while (SDL_PollEvent(&event)) {
			...
			// "R" to restart, space to change direction of ball
				...
				if (event.key.keysym.scancode == SDL_SCANCODE_R) {
					ball.x_pos = 0;
					ball.y_pos = 0;
					Mix_PlayChannel(-1, pongBounce, 0);
				}
		...
		ball.x_pos += elapsed * ball.velocity_x * 0;
		ball.y_pos += elapsed * ball.velocity_y * 0;
